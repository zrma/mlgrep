...
# Mallang is the public compiler distribution consumed by this repository.
PUBLIC_REPOSITORY_DEPENDENCIES = {("zrma", "mallang")}
...
def fixed_patterns(owner: str, repository: str) -> list[tuple[str, re.Pattern[str]]]:
    ...
    allowed.update(
        dependency
        for dependency_owner, dependency in PUBLIC_REPOSITORY_DEPENDENCIES
        if dependency_owner.lower() == owner.lower()
    )
    allowed_repositories = "|".join(
        re.escape(name) for name in sorted(allowed)
    )
    return [
        (
        ...
        (
            ...
            re.compile(rf"(?i)(?:https?://github\.com/|git@github\.com:){re.escape(owner)}/(?!(?:{allowed_repositories})(?:\.git)?(?![A-Za-z0-9_.-]))[A-Za-z0-9_.-]+"),
        ),
        (
            "same-owner-repository-identity",
            re.compile(rf"(?i)(?<![A-Za-z0-9_./\\-]){re.escape(owner)}/(?!(?:{allowed_repositories})(?:\.git)?(?![A-Za-z0-9_.-]))[A-Za-z0-9_.-]+"),
        ),
    ]
